 ...
 *
 * Issue Archive. Version without header or footer
 *
 *}

{assign var="journalPath" value=$currentJournal->getPath()}
...
{iterate from=issues item=issue}
	...
	{if $issue->getFileName($coverLocale) && $issue->getShowCoverPage($coverLocale) && !$issue->getHideCoverPageArchives($coverLocale)}
		<div class="issueCoverImage"><a href="{url op="view" page="issue" context="$journalPath" path=$issue->getBestIssueId($currentJournal)}"><img src="{$coverPagePath|escape}{$issue->getFileName($coverLocale)|escape}"{if $issue->getCoverPageAltText($coverLocale) != ''} alt="{$issue->getCoverPageAltText($coverLocale)|escape}"{else} alt="{translate key="issue.coverPage.altText"}"{/if}/></a>
		</div>
		<h4><a href="{url op="view" page="issue" context="$journalPath" path=$issue->getBestIssueId($currentJournal)}">{$issue->getIssueIdentification()|escape}</a></h4>
